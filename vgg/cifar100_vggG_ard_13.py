...
FLAGS.save_name='cifar100_vggG_ard_13'
...
train_dataset=torchvision.datasets.CIFAR100(root='./data', train=True, transform=train_tfms, download=False)
test_dataset=torchvision.datasets.CIFAR100(root='./data', train=False, transform=test_tfms, download=False)
...
class VGGG_CIFAR100(torch.nn.Module):
    def __init__(self):
        super(VGGG_CIFAR100, self).__init__()        
        convs=make_conv_layers(Gconfig)
        self.convs=torch.nn.Sequential(*convs)
        fcs = [ARDDense(512, 512,FLAGS.init_alpha), torch.nn.ReLU(),
               ARDDense(512, 100,FLAGS.init_alpha)]
...
for aziter in range(azstart,FLAGS.azepoch):
    ...
    if aziter==0:
        resumefile='cifar100_vggG_map_alpha0-statedict.ckpt'
